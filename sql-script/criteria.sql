...
select * from umls.mrrel 
	where (REL='PR' OR REL='PAR')
		and cui2 in (select * from umls.mrconso where str='CHF');


CREATE TABLE demential (
...
load data local infile '/tmp/criteria_demential_trials.csv' 
into table demential
fields terminated by ','
enclosed by '"'
lines terminated by '\r\n'
ignore 1 LINES
(@col1,@col2) set tid=@col1,criteria=@col2 
;

select * from criteria.demential_ytex limit 100;

use ytex;
select * from criteria.demential_ytex;
/*this change to 1004 data set*/
drop table if exists criteria.demential_ytex;
create table criteria.demential_ytex as 
 select d.instance_key as tid, c.code as cui, a.anno_text as target, c.disambiguated,
 c.sentence_text as sentence
  from v_document_cui_sent c 
  inner join  v_annotation a on c.anno_base_id = a.anno_base_id
  inner join document d on d.document_id = c.document_id;
  /*,7230*/

select * from criteria.demential_ytex
	into outfile '/tmp/demential_ytex.csv' fields terminated by ',' enclosed by '"' lines terminated by '\n';  ;
;

select * from ytex.ref_named_entity_regex;
select * from ytex.ref_stopword;
select * from ytex.ref_segment_regex;
select * from ytex.ref_uima_type;

select * from ytex.document limit 100;

insert into ytex.ref_segment_regex (segment_id, regex,limit_to_regex) values('#_#', '#',1);
...
delete from cancer_cui where sty = 'T033'; -- 99002,104777,103562,103383,102505
delete from cancer_cui where sentLen>500; -- 2027,1974,1940,1876
delete from cancer_cui where duration<-1; -- 11,10,10,11
update cancer_cui set month = -1 where duration = -1; -- 456818
...
select count(distinct cui) from cancer_cui;  -- 16443,9687,6620,6437,6434,6434,6435,7071,6637,6552,5127,5114,5105,5103
select count( *) from cancer_cui;  -- 1964495,736162,471374,375966,374944,374943,375021,376239,541486,545457,544044,445042,444495,438812,438064
-- it is less because one cui may occurs multiple times in one criteria.
select count( distinct tid,criteriaId,sentId,termId,cui) from cancer_cui;  -- 1845839,693488,445725,358271,357455,347494,351540,351609,352998,509062,507815,606168,411809,410719,406304,405768
